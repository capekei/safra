...
export function serveStatic(app: Express) {
  const distPath = path.resolve(process.cwd(), "src/client/dist");

  if (!fs.existsSync(distPath)) {
    console.warn(
      `⚠️  Could not find the build directory: ${distPath}. Static files will not be served.`
    );
    console.warn("🔧 Run 'npm run build' to generate client build files");
    return;
  }
